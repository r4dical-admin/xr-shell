...
const tools = [
  ...
  { name: 'xr_shell_open_layout', description: 'Open up to three macOS apps and place them in a specified XR layout. Also creates a draggable, closable A2UI layout controller.', inputSchema: { type: 'object', required: ['apps'], properties: { title: { type: 'string' }, surfaceId: { type: 'string' }, x: { type: 'number' }, y: { type: 'number' }, apps: { type: 'array', minItems: 1, maxItems: 3, items: { type: 'object', properties: { sourceId: { type: 'string' }, query: { type: 'string' }, x: { type: 'number' }, y: { type: 'number' }, width: { type: 'number' }, height: { type: 'number' } }, additionalProperties: false } } }, additionalProperties: false } },
